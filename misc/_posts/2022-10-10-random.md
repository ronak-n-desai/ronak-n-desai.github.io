...
<form id="calc" oninput="calcheight()">
  ...
  <div> 
  <label for="output1">Calculated Height (cm):</label>
  <p class="output" id="output1"></p>
  </div>
  <div> 
  <label for="output2">Total Energy Conservation Height (cm):</label>
  <input type="number" id="output2" readonly></p>
  </div>  
</form>
...
<script>
       ...
       function calcheight() {
              ...
              const h1 = h10.value;
              const l = l0.value;
              out1.innerHTML = -3*(4*h3*h3-l*l)*(-1*12*h1*h3*h3*mO*mO+24*h3*h3*h3*mO*mO+3*h1*l*l*mO*mO+2*h3*l*l*(3*mC*mC-3*mM*mM-mM*mP+mO*mP+mC*(6*mO+mP)))/(4*l*l*l*l*(3*mM+mP)*(3*mM+mP));
