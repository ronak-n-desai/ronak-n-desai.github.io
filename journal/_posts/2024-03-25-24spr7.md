...
![image](https://github.com/ronak-n-desai/ronak-n-desai.github.io/assets/98538788/f098576e-442b-4b52-9702-63ccd304d666)

In the above picture, the mentioned mean is denoted by a star and the labeled points on the scatter plot correspond to the points after filtering for the lowest 25 percentile of distance to the mean. Note that in the case of 0.8 MeV and $\alpha=0.5$, the star does not lie within the scatter plot of points. This is because the optimal region is a horeshoe shape (purple region) and the mean happens to be in a place that isn't actually optimal. This demonstrates why I am choosing not so simply go with the mean. When the optimal region has a non-spherical shape or is segmented into two parts, the mean may not be indicative of an optimal value. Then, among the remaining points, I can sample points randomly to pick out an optimal value to send back to the feedback loop.

In this analysis, I am using the GPR as a regression model. I tried using the SVR, but it does not scale well when running on GPU. Even for the GPR, it doesn't scale well past around 10,000 points.
