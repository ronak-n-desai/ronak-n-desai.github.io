...
Here, $u_{max} = 2 \pi \frac{\lambda_D}{\Delta x} \times R$ where $R$ is some multiplicative factor that tells us where to implement our cutoff. $R = 1$ corresponds to saying that the maximum wavenumber considered is limited the the cell size: $k_{max} = \frac{2 \pi}{\Delta x}$. 
...
\begin{equation}
\frac{\Delta T_{eV}}{\Delta t_{ps}} = C_H \frac{n_{23}^{3/2} \Delta x_{nm}^2}{n_{ppc}} \text{Log}(1 + 4 \pi^2 (\frac{\lambda_D}{\Delta x})^2)
\end{equation}

which is the exact same as the a
