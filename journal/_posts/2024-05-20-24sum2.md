...
It looks like the very beginning of the optimization script saw some pretty high electron counts. A useful thing to do would be to evalute the effectiveness of the optimization, but this is something I can do later. 

# 04-22 Data

![image](https://github.com/ronak-n-desai/ronak-n-desai.github.io/assets/98538788/fc273ccb-cd65-4aed-b331-56164b8cd4df)

There was a shot number error in this data (which I fixed) and now I'm plotting a summary of the run.
...
### Electron Count
I lowered the tolerance from 100,000 to 75,000 for the min electron count so that we don't shave off as many data points. I set the `min_e` default to 75,000 as well to reflect this.
