...
To me, this plot shows a constant drift in time between the clocks of the Pspec and Espec at a rate of approximately (assuming 20Hz data collection) of 30ms drift per 1 second of data collection, which is a pretty significant amount. At the same time, it doesn't really look like there's any freezing going on, so maybe that's just an issue with the LabVIEW visual interface and not with the actual data collection.
